...
function ExecutionContent(): React.ReactElement {
  ...
  const probe = useCallback(async () => {
    ...
    setMessage('')
    try {
      const result = await window.harness.probeExecution(selectedProjectId, provider)
      setCapability(result as unknown as Capability)
    } catch (cause) { setMessage(cause instanceof Error ? cause.message : `${provider} probe failed`) }
  }, [provider, selectedProjectId])
  ...
  async function start(): Promise<void> {
    ...
    const authorization = authorizePolicy(settings.policy.commandExecution, `Start managed ${provider === 'claude' ? 'Claude Code' : 'Codex'} execution for ${activeRun.run_id}?`)
    if (!authorization.allowed) {
      if (authorization.blocked) setMessage(policyBlockedMessage('Managed execution'))
      return
    }
    setLogs([]); setPendingApproval(undefined); setMessage(''); setRunning(true)
    try {
      const runId = activeRun.run_id
      const result = await window.harness.startExecution(selectedProjectId, runId, revision || activeRun.revision || undefined, provider, settings.policy.commandExecution === 'ask')
  ...
  return <section className="page">
    <header className="page-header"><h1>AI Execution</h1><button className="button icon-button" onClick={() => void probe()} title={`${text('Probe', '检测')} ${provider}`}>R</button></header>
    {message && <div className="notice error">{message}</div>}
    <div className="segmented-control" aria-label="Execution provider">
      {(['codex', 'claude'] as AiProvider[]).map((item) => <button key={item} className={provider === item ? 'active' : ''} onClick={() => { setProvider(item); setCapability(undefined); setLogs([]); setMessage('') }}>{item === 'claude' ? 'Claude Code' : 'Codex'}</button>)}
    </div>
    <div className="panel toolbar">
      <span className={`badge ${capability?.available ? 'success' : 'danger'}`}>{capability?.available ? 'AVAILABLE' : 'UNAVAILABLE'}</span>
      <strong>{capability?.version || `${provider === 'claude' ? 'Claude Code managed adapter' : 'Codex'} not detected`}</strong><span className="mono muted">{capability?.path}</span>
    ...
    <div className="actions" style={{ margin: '14px 0' }}>
      <button className="button primary" disabled={running || !capability?.available || !activeRun} onClick={() => void start()}>&gt; {text('Start', '启动')}</button>
      <button className="button danger" disabled={!running} onClick={() => void cancel()}>{text('Stop', '停止')}</button>
    ...
    {pendingApproval && <div className={`notice ${confirmDangerous ? 'error' : ''}`}>
      ...
      <div className="actions">
        <button className="button primary" onClick={() => void respond('allow_once')}>{confirmDangerous ? text('Confirm allow', '确认允许') : text('Allow once', '仅允许一次')}</button>
        <button className="button" onClick={() => void respond('allow_session')}>{text('Allow session', '本次会话允许')}</button>
        <button className="button danger" onClick={() => void respond('deny')}>{text('Deny', '拒绝')}</button>
      </div>
    </div>}
