...
const harnessApi: HarnessApi = {
  ...
  setAppSettings: (settings) => ipcRenderer.invoke('app-settings:set',settings),
  ...
  probeGithubRelease: (p: string,t: string) => ipcRenderer.invoke('release:probe',p,t),
  selectGithubReleaseAssets: () => ipcRenderer.invoke('release:select-assets'),
  publishGithubRelease: (p: string,r,c?: boolean) => ipcRenderer.invoke('release:publish',p,r,c),
  ...
  preflightRunMergeBack: (p: string,id: string,rev?: string) => ipcRenderer.invoke('run:merge-back-preflight',p,id,rev),
  mergeRunBack: (p: string,id: string,rev?: string,confirmed?: boolean) => ipcRenderer.invoke('run:merge-back',p,id,rev,confirmed),
  ...
  getAiCliSettings: (provider) => ipcRenderer.invoke(`${provider}-settings:get`),
  discoverAiCli: (provider) => ipcRenderer.invoke(`${provider}-settings:discover`),
  selectAiCliExecutable: (provider) => ipcRenderer.invoke(`${provider}-settings:select`),
  setAiCliExecutable: (provider,path) => ipcRenderer.invoke(`${provider}-settings:set-path`,path),
  ...
  startKnowledgeCodexSynthesis: (p: string,ids: number[],allowDirty?: boolean,confirmed?: boolean,dirtyConfirmed?: boolean,provider?: 'codex' | 'claude') => ipcRenderer.invoke('knowledge:repo-codex-start',p,ids,allowDirty,confirmed,dirtyConfirmed,provider),
  ...
  pushKnowledgeRepo: (p: string,ids: number[],pushConfirmed?: boolean,commitConfirmed?: boolean) => ipcRenderer.invoke('knowledge:repo-push',p,ids,pushConfirmed,commitConfirmed),
  probeExecution: (p: string,provider?: 'codex' | 'claude') => ipcRenderer.invoke('execution:probe',p,provider),
  startExecution: (p: string,r: string,rev?: string,provider?: 'codex' | 'claude',confirmed?: boolean) => ipcRenderer.invoke('execution:start',p,r,rev,provider,confirmed),
  ...
  archiveRecoverySession: (p: string,s: string) => ipcRenderer.invoke('recovery:archive',p,s),
